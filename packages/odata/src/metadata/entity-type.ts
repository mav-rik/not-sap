...
> {
  ...
  constructor(
    ...
    if (kind === 'entity') {
      ...
      throw new Error(
        `"${_entityType as string}" does not exist in metadata of the service "${_m.name}".`
      );
    }
  ...
  private _updateNavProps() {
    ...
    for (const np of this._type.NavigationProperty || []) {
      if (this._m.isV4) {
        ...
        const navProperty: NavProperty<
          keyof (M['entityTypes'][T]['navToMany'] | M['entityTypes'][T]['navToOne'])
        > = {
          $Name: navProp.$Name as keyof (
            | M['entityTypes'][T]['navToMany']
            | M['entityTypes'][T]['navToOne']
          ),
        ...
        if (association) {
          ...
          if (targetEnd) {
            ...
            if (isToMany) {
              this._navToMany!.push(
                navProperty as NavProperty<keyof M['entityTypes'][T]['navToMany']>
              );
            } else {
              this._navToOne!.push(
                navProperty as NavProperty<keyof M['entityTypes'][T]['navToOne']>
              );
            }
            this._navsMap.set(navProperty.$Name, navProperty);
          }
  ...
  get selfDataType() {
    return this._m.isV4 ? `SAP__self.${this._type.$Name}` : this.name;
  }
  ...
  prepareQuery(
    ...
    const _expand = params.expand
      ? typeof params.expand === 'string'
        ? params.expand
        : Array.isArray(params.expand)
          ? params.expand.map((e) => e.render())
          : [params.expand.render()]
      : undefined;

    const expand =
      typeof _expand === 'string'
        ? _expand
        : _expand
          ? _expand.map((e) => e.expandString).join(',')
          : undefined;
    let filter = params.filter
      ? this.renderFilter(params.filter, undefined, expandPrefix) || undefined
      : undefined;
    const _select = select ?? ([] as string[]);

    if (!isV4 && Array.isArray(_expand)) {
      const filters = [filter, ..._expand.map((e) => e.filterV2)];
      filter = filters
        .filter(Boolean)
        .map((f) => `(${f})`)
        .join(' and ');
  ...
  expand<NT extends keyof M['entityTypes'][T]['navToMany'] | keyof M['entityTypes'][T]['navToOne']>(
    ...
    params?: TEntitySetQueryParams<
      M,
      NT extends keyof M['entityTypes'][T]['navToMany']
    ...
    return new EntityExpand<M, newType>(this._m, newEntityType as newType, {
      property: navProp as string,
      params,
    });
  }
      ...
      .map((_f) => {
        ...
        for (const [field, val] of Object.entries(
          ...
          if ((type === 'empty' || type === 'notEmpty') && meta && meta.$Type === 'Edm.String') {
            ...
            fieldFilters.push(
              odataFilterFormat.toFilter[type](
                fieldPath,
