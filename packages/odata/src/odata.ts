...
export class OData<M extends TOdataDummyInterface = TOdataDummyInterface> {
  ...
  async read<T extends keyof M['entitySets']>(
    ...
    const data = await this._fetch<
      TWrapODataResults<M['entityTypes'][M['entitySets'][T]]['record']>
    >(
    ...
    const run = async () => {
      try {
        ...
        const chunks = [] as Promise<
          TOdataReadResult<M['entityTypes'][M['entitySets'][T]]['record']>
        >[];
  ...
  async readRecordByKey<T extends keyof M['entitySets']>(key: string) {
    const data = await this._fetch<
      TWrapODataRecord<M['entityTypes'][M['entitySets'][T]]['record']>
    >(
  ...
  async entitySet<T extends keyof M['entitySets']>(
    name: T
  ): Promise<EntitySet<M, T, M['entitySets'][T]>> {
    const m = await this.getMetadata();
  ...
  protected async _callFunction(
    ...
    if (_params) {
      for (const param of fnMeta.params ?? []) {
        ...
        if (paramName in (_params as Record<string, any>)) {
          ...
          params[param.$Name] =
            target === 'filter'
              ? odataValueFormat.toFilter[param.$Type as 'Edm.String'](value as string)
    ...
    if (target === 'filter') {
      ...
      return isV4
        ? (data as any).value
        : isCollection
          ? (data as any).d?.results || (data as any).d
          : (data as any).d;
    }
  }
