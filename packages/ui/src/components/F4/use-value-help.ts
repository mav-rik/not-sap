import type { EntitySet, TEntitySetQueryParams } from '@/_odata/metadata/entity-set'
import type { OData, TOdataDummyInterface, TODataTypedFilterValue } from '@/_odata'
import { useSmartTablePI } from '../SmartTable/SmartTable.pi'
import type { TPropertyAnnotations } from '@/_odata/metadata/annotations'
...
>(fieldName: string) {
  ...
  const updateVars = () => {
    entity.value
      ...
      .then(es => {
        ...
        valueList.value = entity.value?.getField(fieldName as M['entitySets'][K]['fields'])
  ...
  const inParams = computed(
    () =>
      valueList.value?.Parameters.filter(p => p.LocalDataProperty && inFields.includes(p.$Type)) ||
      []
  )
