import type {
  ...
  TODataFilters,
  TODataParams,
  TODataTypedFilterValue,
} from '@/_odata'
import { useProvideInject } from 'vunor/utils'
import type { ModelRef, Ref, ComputedRef, Reactive } from 'vue'
import { nextTick } from 'vue'
import { convertF4Condition, fieldsFiltersToODataFilters } from './filter-conditions'
import { useODataEntitySetPI } from '../../renderless/ODataEntitySet.pi'
import { debounce } from '../debounce'
import { NotSapPresets, type TNotSapPreset } from '@/_not-sap-ui/presets/presets'
import { parsePresetField, stringifyPresetField } from '@/_not-sap-ui/presets'
    ...
    return {
      ...
      _provide: (opts: {
        ...
        if (toInject.forceSorters.value) {
          toInject.sorters.value = mergeSorters(toInject.forceSorters.value, toInject.sorters.value)
        }
        ...
        async function updateFavPresets(favs?: string[]) {
          ...
          if (appNamespace?.value) {
            const p = new NotSapPresets(appNamespace.value, toInject.variantKey.value)
            await p.setFavPresets(favs || toInject.presets.value.filter(p => p.fav).map(p => p.id))
          }
        }
